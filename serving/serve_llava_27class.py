"""Serve fine-tuned LLaVA-1.5-7B as a 27-class alignment predictor over HTTP.

The VLM half of the model comparison. Shells out to run_llava.py for
each request, matches the generated text against the 27 label strings, and
answers on the key "movement" so the robot clients cannot tell it apart from
the CNN server.

The QUERY string must match the prompt the adapter was fine-tuned with. Around
6-7 s per request, against under one second for the CNN.

Defaults to port 5000, which collides with the ViRA fine-direction server.
"""
...
MODEL_PATH = os.environ.get("MODEL_PATH", "checkpoints/llava-v1.5-7b-opentron-lora")
MODEL_BASE = os.environ.get("LLAVA_BASE", "pretrained/llava-v1.5-7b")
