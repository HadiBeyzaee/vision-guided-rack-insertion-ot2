"""Turn commanded offsets into the LLaVA conversation JSON.

Step 4 of the dataset pipeline. Joins image_rename_mapping.json to
error_data.txt by row index, converts each (dx, dy, dtheta) to one of the 27
correction strings, and emits one conversation per augmented image.

Thresholds: 0.5 mm translation dead-band, 0.2 deg yaw dead-band. Because the
offset was commanded, no image is ever hand-labelled.

The question text here must match the --query the inference server sends.
"""
