"""Train the 8-class Coarse/Fine granularity classifier.

The CNN counterpart to the ViRA phase-switch decision: 2 x 2 x 2 Coarse/Fine
labels, one per axis, not the 27-class direction head. Same backbone and head
shape as train_cnn_27class.py otherwise.

KNOWN ISSUE, left as found: the dataset is built with augment=True and never
rebound, so appearance augmentation is applied to validation and test as well
as training. See docs/caveats.md.
"""
...
json_file_path = os.environ.get(
    "DATASET_JSON", "data/cnn_coarse_fine_dataset.json")
CHECKPOINT_PATH = os.environ.get(
    "CHECKPOINT_PATH", "checkpoints/cnn_coarse_fine_classifier.pth")

dx_labels = ["Fine", "Coarse"]
dy_labels = ["Fine", "Coarse"]
dtheta_labels = ["Fine", "Coarse"]

class_labels = [
    f"{x}, {y}, {r}"
    for x in dx_labels
    for y in dy_labels
    for r in dtheta_labels
]
...
# Model for the 8-class coarse/fine head (2 x 2 x 2), not the 27-class direction head
class ClassificationModel(nn.Module):
    def __init__(self, model_type='resnet', pretrained=True):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU,
            nn.Linear(1024, len(class_labels))  # 8 coarse/fine classes
        )
