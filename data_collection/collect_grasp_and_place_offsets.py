"""Collect the two-source offset dataset for the single-stage study.

The dataset behind the single-stage study. Each trial has TWO independent sources of
error, which is what distinguishes it from the ViRA collection:

  1. a grasp offset dx applied before the gripper closes. The fingers close
     along y, so an offset along x survives closure and is carried into the
     transfer motion.
  2. a placement offset (dxx, dyy, dtheta) applied after transfer, above the
     target slot.

Only their combination is visible in the image. The collector writes all of it,
and training/dataset/compute_effective_dx.py reduces it to the three columns the
dataset builders read.

Ranges as implemented: dx in U(0, 8 mm) one-sided, dxx and dyy in +/-[6, 15] mm,
dtheta in +/-5 deg. The chapter describes dx as 50 discrete values spanning
+/-1 cm; that version is present but commented out.

This file was previously named collect_failed_grasp_dataset.py, which
misdescribed it - it is not a failure-class collector.

Run:    python3 collect_grasp_and_place_offsets.py
...
for trial in range(20):
    ...
    def save_images2(color_image1, depth_image1, index, base_dir):
        # Directory paths
        color_dir1 = os.path.join(base_dir, 'color_images', 'camera2_align')

        color_mapped_dir1 = os.path.join(base_dir, 'color_mapped_depth_images', 'camera2_align')
    ...
    # dx_values_uniform = np.linspace(-0.008, 0.008, 50)  # 50 uniform dx values

    # if trial < 50:
    # dx = dx_values_uniform[trial]
    # else:
    # dx = random.uniform(-0.008, 0.008)  # random dx for later trials


    dx = random.uniform(0.0, 0.008)

    print(f"\nTrial {trial + 1}/100 | dx = {dx:.5f}")

    position = [0.683 + dx, 0.128, 0.253]
    ...
    try:
        ...
        for i in range(50):
            ...
            # dxx = random.uniform(-0.005, 0.005)
            # dyy = random.uniform(-0.005, 0.005)

            dtheta = random.uniform(-5.0, 5.0)

            if random.choice([True, False]):
                dxx = random.uniform(0.006, 0.015)
            else:
                dxx = random.uniform(-0.015, -0.006)

            if random.choice([True, False]):
                dyy = random.uniform(0.006, 0.015)
            else:
                dyy = random.uniform(-0.015, -0.006)




            error_data = [dx, dxx, dyy, dtheta]  # or whatever you define for dx

            # position = [0.681 + dxx, 0.128 + dyy, 0.25]
            position = [0.683 + dxx, 0.128 + dyy, 0.25]
            euler_angles = [-180, 0,-1.4 + dtheta]  # roll, pitch, yaw
        ...
        for i in range(25):
            dxxx = random.uniform(-0.0003, 0.0003)
            dyyy = random.uniform(-0.0003, 0.0003)
            dthetaa = random.uniform(-0.03, 0.03)

            position = [0.683 + dx + dxxx, 0.128 + dyyy, 0.25]
            euler_angles = [-180, 0,-1.4 + dthetaa]  # roll, pitch, yaw
