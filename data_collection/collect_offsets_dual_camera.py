"""Collect the labelled offset dataset on the Opentrons deck - two cameras.

The training data behind every learned model in this repo. For 300 iterations:
move to the taught nominal pose, apply a random offset of +/-13 mm in x and y
and +/-4 degrees in yaw, capture RGB and depth from both cameras, and save.

Layout produced under opentron/:
  color_images/cameraN/<i>.png
  grayscale_depth_images/cameraN/<i>.png
  color_mapped_depth_images/cameraN/<i>.png
  error_data.txt              one "dx dy dtheta" line per sample

The image index comes from the row count of error_data.txt, so an interrupted
run resumes cleanly - the label file is the source of truth.

Run:    python3 collect_offsets_dual_camera.py
...
def save_images(color_image1, color_image2, depth_image1, depth_image2, index, base_dir):
    # Directory paths
    ...
    process_and_save_depth_images(depth_image2, gray_dir2, color_mapped_dir2, index)
...
for i in range(300):
    ...
    dx = random.uniform(-0.013, 0.013)  # -0.05 to 0.05 meters
    dy = random.uniform(-0.013, 0.013)  # -0.05 to 0.05 meters
    #dz = random.uniform(-0.01, 0.01)
    dtheta = random.uniform(-4.0, 4.0)    # -5 to 5 degrees
    ...
    try:
        ...
        if image_subscriber.image1 is not None and image_subscriber.image2 is not None and image_subscriber.depth_image1 is not None and image_subscriber.depth_image2 is not None:
            ...
            save_images(image_subscriber.image1, image_subscriber.image2, image_subscriber.depth_image1, image_subscriber.depth_image2, number_of_image, base_dir)
