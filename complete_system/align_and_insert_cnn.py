"""Close the loop on the rack pose with the CNN direction classifier, then insert.

Streams frames from the RealSense, crops to the region the model was trained
on, POSTs each to the CNN server, and applies the returned direction as a fixed
dx/dy/dtheta step. When the model returns "No Move" consistently, drops z to
seat the rack and releases.

Called as a subprocess by run_end_to_end_tasks.py, but runs standalone too.

Run:    python3 align_and_insert_cnn.py
