"""Carry the grasped rack to a named Opentrons deck slot.

move_to_slot(slot_name) lifts clear of the table, then follows a fixed
joint-space waypoint path to the slot. The waypoints exist because a direct
Cartesian move clips the deck wall on the way in - do not replace them with a
straight-line move without re-checking clearance.

Slots are named "slot1" through "slot6".
"""
...
def move_to_slot(slot_name: str):
    # ---------------- Robot ----------------
    ...
    new_position[2] += 0.1
    panda.move_to_pose(new_position, panda.get_orientation,  speed_factor=SPEED, stiffness=stiffness)
    ...
    MIDPOINTS = [ MIDPOINT_2, MIDPOINT_3, MIDPOINT_4]
    ...
    """Move robot to a specified slot (slot1..slot6 or just 1..6)."""
    try:
