"""Closed-loop alignment driven by the VLM instead of the CNN.

The other half of the CNN-versus-VLM comparison. Identical loop to
align_and_insert_cnn.py - same `camera2` top-down crop {240, 320, 500, 200},
same 27-class correction strings, same stopping rule - but it posts to the
LLaVA server on port 5000 rather than the CNN server on 4001.

Steps are 1.1 mm in x and y and 0.1 deg in yaw, dropping 0.08 m to seat the
rack. The practical difference is latency: roughly 6-7 s per prediction against
under a second for the CNN, which is what makes the CNN the better fit for a
closed loop despite both sharing an output space.
