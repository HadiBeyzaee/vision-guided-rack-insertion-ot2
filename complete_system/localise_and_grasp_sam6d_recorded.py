"""As localise_and_grasp_sam6d.py, but records the approach on video.

Same pose estimation and grasp, with one difference that matters in practice:
the RealSense is opened twice. First at 640x480 to capture the frame the SAM-6D
server expects, then restarted at 1280x720 to record the grasp on a background
thread. Use this version when producing figures or video for a write-up.
"""
