"""Insertion driven by a single VLM movement label - the baseline version.

One server call per iteration returns one label, which maps to a fixed
dx/dy/dtheta step. No coarse/fine logic, no per-axis split. Start here when
reading this line of work; the other two files are refinements of it.
"""
...
class DualSubscriptionNode(Node):
    ...
    def move_robot(self, movement_prediction):
        pose = panda.get_pose()
        position = [pose[0, 3], pose[1, 3], 0.16]
