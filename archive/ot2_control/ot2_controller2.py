...
def run(protocol: protocol_api.ProtocolContext):
    ...
    protocol.comment("Moving to Abgene Slot 2, A1...")
    pipette.move_to(abgene["A1"].top())
    protocol.delay(seconds=1)

    # Go into well
    protocol.comment("Dispensing into Abgene A1...")
    ...
    protocol.comment("Task 1 complete!")
'''

# TASK 2: Pick tip from Slot 1 (A1 - new rack), go to Slot 3 (Nunc), dispense, drop tip
TASK_2_PROTOCOL = '''
from opentrons import protocol_api

metadata = {
    "protocolName": "Task 2 - Tip to Nunc Wellplate",
    "author": "Panda",
    "apiLevel": "2.15"
}
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    # Slot 3: Nunc Denmark shallow wellplate
    nunc = protocol.load_labware("corning_96_wellplate_360ul_flat", "3")

    # Load pipette
    pipette = protocol.load_instrument("p300_single_gen2", "left")

    # Pick up tip from A1
    protocol.comment("Picking up tip from Slot 1, A1 (new rack)...")
    pipette.pick_up_tip(tip_rack["A1"])
    protocol.delay(seconds=1)

    # Move to Nunc plate A1
    protocol.comment("Moving to Nunc Slot 3, A1...")
    pipette.move_to(nunc["A1"].top())
    protocol.delay(seconds=1)

    # Go into well
    protocol.comment("Dispensing into Nunc A1...")
    pipette.dispense(100, nunc["A1"])
    protocol.delay(seconds=1)
    ...
    protocol.comment("Task 2 complete!")
'''
...
def main():
    ...
    send_slack(
        ...
        "------------------------------------\n"
        "Task 1: Tip -> Abgene (Slot 2)\n"
        "Task 2: Tip -> Nunc (Slot 3)\n"
        "Rack replacements: 2\n"
        "------------------------------------"
    )

    # =============================================
    # TASK 1: Pick tip, go to Abgene
    # =============================================
    ...
    # =============================================
    # TASK 2: Pick tip from NEW rack, go to Nunc
    # =============================================
    send_slack(" *OT-2*: *TASK 2* - Picking tip from Slot 1 (new rack), dispensing to Slot 3 (Nunc)...")
    ...
    # =============================================
    # REPLACEMENT 2: Replace Slot 3 Nunc plate
    # =============================================
    send_slack(" *OT-2*: Pausing for Panda to replace Slot 3...")
    pause_for_panda()

    request_rack_replacement(
        slot_num=3,
        rack_type="Wellplate (Nunc shallow)"
    )
