...
def run_pose_estimation_with_segmentation(rack_color="blue", rack_real_name="Blue Tip Rack"):
    ...
    try:
       ...
       # pipeline.stop
        ...
        # Around line 147 in your robot PC code
        result = segment_rack_via_server(
            rgb_path=rgb_path,
            rack_color=rack_color,
            rack_name=rack_real_name,
            json_output_path=None  # Don't specify path
        )
        ...
        json_data = result['json_data']  # This is the actual JSON content

        # Save JSON data locally on robot PC
        local_json_path = f"/tmp/{rack_color}_rack_seg.json"  # Local temp path
        with open(local_json_path, 'w') as f:
            json.dump(json_data, f, indent=2)

        print(f"JSON saved locally -> {local_json_path}")

        # Save segmented RGB (optional, for inspection)
        ...
        files = {
            'rgb': open(rgb_path, 'rb'),              # Original RGB
            'depth': open(depth_path, 'rb'),          # Depth image
            'seg_json': open(local_json_path, 'rb')   # LOCAL JSON path
        }
