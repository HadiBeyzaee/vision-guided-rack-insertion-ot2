...
class StackedImageDataset(Dataset):
    def __init__(self, image_dirs, error_data_files, transform=None, augment_times=10):
        ...
        for image_dir, error_data_file in zip(image_dirs, error_data_files):
            ...
            for i in range(len(error_data)):
                img_name = f"{i+1}.png"
                dx_value = float(error_data.iloc[i, 0])  # <- safely extract float dx
                for _ in range(augment_times):
                    self.image_info.append((os.path.join(image_dir, img_name), dx_value))
    ...
    def __getitem__(self, idx):
        ...
        return image, torch.tensor([dx_value], dtype=torch.float32)  # ensure shape [1]
...
val_test_transform = transforms.Compose([
    transforms.Resize((250,250)),  # Resize the image
    transforms.ToTensor,  # Convert PIL image to tensor
])

# Paths
image_dirs = [
    os.path.join(BASE_DIR, 'external_wrong1/color_images/camera1_cropped1'),
    os.path.join(BASE_DIR, 'external_wrong2/color_images/camera1_cropped1'),
    os.path.join(BASE_DIR, 'external_wrong4/color_images/camera1_cropped1'),
]
error_data_files = [
    os.path.join(BASE_DIR, 'external_wrong1/error_data.txt'),
     os.path.join(BASE_DIR, 'external_wrong2/error_data.txt'),
     os.path.join(BASE_DIR, 'external_wrong4/error_data.txt'),
]

# Dataset and DataLoader
full_dataset = StackedImageDataset(image_dirs, error_data_files, transform=None, augment_times=10)
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        torch.save(model.state_dict, 'train_reg_crop1_wrong_new1242.pth')

    scheduler.step


# Load the best model for testing
model.load_state_dict(torch.load('train_reg_crop1_wrong_new1242.pth'))
model.eval
