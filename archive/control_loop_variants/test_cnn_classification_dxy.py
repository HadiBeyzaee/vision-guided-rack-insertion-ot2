...
FLASK_SERVER_URL = f"http://{INFERENCE_HOST}:4010/predict"
...
class DualSubscriptionNode(Node):
    ...
    def prediction_callback(self, msg):
        ...
        try:
            ...
            if prediction.strip() == "No Move, No Move":
    ...
    def move_robot(self, movement_prediction):
        ...
        parts = [x.strip() for x in movement_prediction.split(",")]
        if len(parts) != 2:
            self.get_logger().warning(f"Invalid prediction format: '{movement_prediction}'")
            return

        dx_label, dy_label = parts
        ...
        pose[0, 3], pose[1, 3] = position[0] , position[1]
        pose[2, 3] = 0.25

        self.get_logger().info(f"Moving: {dx_label}, {dy_label}")
