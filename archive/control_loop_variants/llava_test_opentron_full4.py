...
FLASK_SERVER_URL = f"http://{INFERENCE_HOST}:5004/predict"
...
class DualSubscriptionNode(Node):
    ...
    def lower_z_and_schedule_timer(self):
        ...
        # euler_angles[0] = euler_angles[0] - 7.5
        # euler_angles_new = [euler_angles[0], euler_angles[1], euler_angles[2]]  # roll, pitch, yaw
        # rotation_matrix = euler_to_rotation_matrix(*euler_angles_new)
        # pose[0, 0:3] = rotation_matrix[0, 0:3]
        # pose[1, 0:3] = rotation_matrix[1, 0:3]
        # pose[2, 0:3] = rotation_matrix[2, 0:3]
        # pose[2, 3] = pose[2, 3]- 0.01
        # panda.move_to_pose(pose, speed_factor=speed_factor)

        # pose = panda.get_pose()
        # pose[1, 3] = pose[1, 3]- 0.05  # y-coordinate
        # panda.move_to_pose(pose, speed_factor=speed_factor)
        ...
        # pose[1, 0:3] = rotation_matrix[1, 0:3]
        # pose[2, 0:3] = rotation_matrix[2, 0:3]
        # pose[2, 3] = pose[2, 3]- 0.01  # y-coordinate
        # panda.move_to_pose(pose, speed_factor=speed_factor)
        ...
        # pose = panda.get_pose()    
        # pose[1, 3] = pose[1, 3] + 0.05  # y-coordinate
        # pose[2, 3] = pose[2, 3] + 0.07  # y-coordinate
        # panda.move_to_pose(pose, speed_factor=speed_factor)
        # gripper.grasp(0.0, 0.2, 400, 0.04, 0.04)
