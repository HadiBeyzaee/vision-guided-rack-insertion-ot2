...
FLASK_SERVER_URL = f"http://{INFERENCE_HOST}:5092/predict"
FLASK_SERVER_URL2 = f"http://{INFERENCE_HOST}:5012/predict"
FLASK_SERVER_URL3 = f"http://{INFERENCE_HOST}:5003/predict"
...
class DualSubscriptionNode(Node):
    ...
    def get_movement_amount_flags(self, image_path):
        with open(image_path, "rb") as image_file:
            files = {"image": image_file}
            try:
                response = requests.post(FLASK_SERVER_URL2, files=files, timeout=10)
                if response.status_code == 200:
    ...
    def prediction_callback(self, msg):
        ...
        try:
            ...
            if not self.skip_amount_check:
                ...
                self.dx_step = 0.007 if dx_big else 0.0008
                self.dy_step = 0.007 if dy_big else 0.0008
                self.dtheta_step = 1.0 if dtheta_big else 0.1
