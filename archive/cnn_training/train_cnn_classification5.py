...
# Define Paths
json_file_path = os.path.join(BASE_DIR, "cnn_augmented2_camera2_crop1_dxyt_llava.json")
...
class ClassificationModel(nn.Module):
    def __init__(self, model_type='resnet', pretrained=True):
        ...
        # Multimodal projector (like LLaVA's MLP with GELU)
        ...
        # Final classifier for 27 movement classes
        self.classifier = nn.Linear(1024, len(combined_labels))

    def forward(self, x):
        features = self.backbone(x)              # CNN features
        features = torch.flatten(features, 1)
        proj_out = self.projector(features)      # Projected features
        return self.classifier(proj_out)         # Classification


# Train Model
def train_model(model, train_loader, val_loader, num_epochs=12, lr=0.0001, device="cuda"):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict, "cnn_augmented2_camera2_crop1_vgg_vlm_twin.pth")
...
model.load_state_dict(torch.load("cnn_augmented2_camera2_crop1_vgg_vlm_twin.pth"))
