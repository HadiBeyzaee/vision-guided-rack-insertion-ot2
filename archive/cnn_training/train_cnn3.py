...
# Define Paths
json_file_path = os.path.join(BASE_DIR, "cnn_camera1_crop2_nine_merged.json")

# Generate all 27 class labels in the correct order
movement_x = ["Move Up", "Move Down"]
movement_y = ["Move Left", "Move Right"]
...
class ClassificationModel(nn.Module):
    def __init__(self, model_type='resnet', pretrained=True):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU,
            nn.Linear(1024, len(class_labels))  # 27-class output
        )
...
def train_model(model, train_loader, val_loader, num_epochs=20, lr=0.0001, device="cuda"):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict, "cnn_camera1_crop2_nine.pth")
...
model.load_state_dict(torch.load("cnn_camera1_crop2_nine.pth"))
