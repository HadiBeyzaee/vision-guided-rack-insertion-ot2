"""The VLM harness with a CNN behind the endpoint instead of LLaVA.

Identical control loop, video overlay and convergence logic - only the server
differs. Kept because it is the direct A/B comparison against
insert_with_llava_single_label.py: same code, same steps, different model.
"""
...
FLASK_SERVER_URL = f"http://{INFERENCE_HOST}:4002/predict"
...
class DualSubscriptionNode(Node):
    def __init__(self):
        ...
        #self.video_path = f"videos/camera1_session_{timestamp}.mp4"
    ...
    def move_robot(self, movement_prediction):
        ...
        try:
            dx_label = movement_prediction.strip()
        ...
        self.get_logger().info(f"Moving: {dx_label}")
