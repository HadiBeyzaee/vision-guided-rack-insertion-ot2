"""ViRA fine phase: the final corrections, including when to stop. Port 5000.

Answers on the key "movement" with the full label set including "No Move" and
"No Rotate", plus lower-case spelling variants the fine-tuned model sometimes
emits. The control loop lowers the rack once this returns
...
# Model configuration
model_path = os.path.join(CHECKPOINT_DIR, "llava-v1.5-7b-opentron-camera1-augmented-crop1-high-angle-epoch-twenty-lora")
model_base = LLAVA_BASE

# Define label set (dx, dy, dtheta based)
movement_x = ["Move Up", "Move Down", "No Move"]  # dx (vertical)
movement_y = ["Move Left", "Move Right", "No Move"]  # dy (horizontal)
...
query_combined = "what movement and what rotation are needed to align it properly?"
